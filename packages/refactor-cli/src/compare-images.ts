...
import { IFileAdapter } from './file-adapters/IFileAdapter';
import { LocalAdapter } from './file-adapters/local-adapter';
  ...
  for (const index in config.pages) {
    ...
    try {
      ...
      if (!pass) {
        ...
        await localAdapter.writeFile(config, 'diff', `${ view.id }.png`, PNG.sync.write(diff));
      }
